...
int main()
{
   ...
   int clientnum, done = 1, i = 0, j = 0, min, max;
   struct Client cli[100];
   FILE * inputfile = fopen("clients.txt", "r");
   FILE * exportfile = fopen("clients_sorted.txt", "w");
   ...
   while(done != 0)
   {
      ...
      switch(choice)
      {
         ...
         case 's':
            ...
            break;
         case 'i':
            fscanf(inputfile, "%i%*c", &clientnum);
            i = 0;
            while(i < 100)
            {
               /* Stops Reading When Number of Clients Reached */
               ...
               /* Gets BMI */                                                                             
               fscanf(inputfile, "%lf*c", &cli[i].bmi);                                                               
 
               /* Gets Fitness Level */                                                                   
               fscanf(inputfile, "%i%*c", &cli[i].fitlvl);                                                            
               i++;
            }
            printf("\nImported Clients.\n\n");
            break;
         case 'e':
            ...
            {
               fprintf(exportfile, "%s -- BMI %.1lf -- Fitness Level %i\n", cli[j].name, cli[j].bmi, cli[j].fitlvl);
            }
            printf("\nExported Clients.\n\n");
            break;
         case 'q':
